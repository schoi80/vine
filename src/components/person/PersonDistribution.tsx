...
interface PersonDistributionProps {
  slug: string;
}

export function PersonDistribution({ slug }: PersonDistributionProps) {
  ...
  const { data, loading } = useQuery(PERSON_VERSES_QUERY, {
    variables: {
      slug,
      first: 500, // Fetch more verses for accurate distribution aggregation
    },
    skip: !slug,
